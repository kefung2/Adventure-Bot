import os
import discord
import monster
import player
import shop

client = discord.Client()

#the global var
curMod = None
curShop = None
curPlayer = None


def helpMenu():
    return ("Coming Soon \n" "Please wait")


@client.event
async def on_ready():
    print('{0.user} Online'.format(client))


@client.event
async def on_message(message):
    if message.author == client:
        return
    # shorthand for code
    msg = message.content
    sendBack = message.channel.send

    # Commend to see menu
    if msg.startswith('$help'):
        menu = helpMenu()
        await sendBack(menu)

    # Commend to see menu
    if msg.startswith('$newPlayer'):
        statList = msg.split()
        for i in range(2, len(statList)):
            statList[i] = int(statList[i])
        print(statList)
        print(message.author)
        if statList[2] + statList[3] + statList[4] + statList[5] + statList[
                6] > 20:
            await sendBack("stat too high")
        elif statList[2] + statList[3] + statList[4] + statList[5] + statList[
                6] < 20:
            await sendBack("you still have stat point to use")
        elif statList[2] + statList[3] + statList[4] + statList[5] + statList[
                6] == 20:
            global curPlayer
            curPlayer = player.player(statList[1], statList[2], statList[3],
                                      statList[4], statList[5], statList[6])

    # Commed to move / go next
    if msg.startswith('$move'):
        menu = helpMenu()
        await sendBack(menu)

    # Commend to fight
    if msg.startswith('$fight'):
        menu = helpMenu()
        await sendBack(menu)

    # Commend to run from fight
    if msg.startswith('$run'):
        menu = helpMenu()
        await sendBack(menu)

    # Commend to buy item
    if msg.startswith('$buy'):
        try:
            item = int(msg.split("$buy", 1)[1])
            await sendBack(item)
        except:
            await sendBack("Please pick 1 item only")

    # Commend to see current monster stat
    if msg.startswith('$monStat'):
        menu = helpMenu()
        await sendBack(menu)

    #commend to see yoour stat
    if msg.startswith('$stat'):
        stat = curPlayer.showStat()
        await sendBack(stat)

    # Comment to reset everything
    if msg.startswith('$reset'):
        menu = helpMenu()
        await sendBack(menu)


client.run(os.environ['TOKEN'])
print("everything is ok")
